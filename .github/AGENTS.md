...
This file guides changes under `.github/` for `komari-theme-natrue`. Keep edits focused on the existing CI workflow and the issue templates. Do not add general source-code rules here.
...
- Run `pnpm install`
- Run `pnpm build`
- Upload artifacts matching `komari-theme-natrue-build*.zip`

## CI editing expectations

Preserve the current artifact naming pattern exactly. Downstream usage and release downloads expect `komari-theme-natrue-build*.zip`.
...
- artifact uploads still use `komari-theme-natrue-build*.zip`
